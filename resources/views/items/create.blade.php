...
@section('title', 'Tambah Barang Lab')

@section('content')
    <div class="mb-6">
        <a href="{{ route('items.index') }}" class="text-sm font-medium text-slate-500 hover:text-slate-700">← Kembali ke data barang</a>
        <h1 class="text-2xl font-bold text-slate-900 mt-2">Tambah Barang Lab</h1>
        <p class="text-sm text-slate-500 mt-1">Lengkapi informasi barang baru untuk laboratorium.</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6 max-w-2xl">
        <form action="{{ route('items.store') }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <x-form-input name="code" label="Kode Barang" placeholder="mis. LC-001" value="{{ old('code') }}" required />
                <x-form-input name="category" label="Kategori" placeholder="mis. Hardware" value="{{ old('category') }}" required />
            </div>

            <x-form-input name="name" label="Nama Barang" placeholder="mis. Laptop Acer TravelMate" value="{{ old('name') }}" required />

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <x-form-input name="stock" label="Jumlah Stok" type="number" min="0" value="{{ old('stock', 0) }}" required />
                <x-form-input name="unit" label="Satuan" placeholder="unit" value="{{ old('unit', 'unit') }}" required />
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          ...
                          placeholder="Keterangan singkat tentang barang...">{{ old('description') }}</textarea>
            </div>
